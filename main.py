...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.store_name = '_'.join(['CF', args.dataset, args.modality, 'segment%d' % args.num_segments, 'e{}'.format(args.epochs)])
    ...
    model = TSN(num_class, args.num_segments, img_feature_dim=args.img_feature_dim,
                dropout=args.dropout,
                partial_bn=not args.no_partialbn, print_spec=True, fc_lr5=True, modality='HP')
...
def validate(val_loader, model, criterion, epoch, args, rank, log=None, tf_writer=None):
    ...
    if rank == 0 and tf_writer is not None:
