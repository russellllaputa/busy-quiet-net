...
class TSN(nn.Module):
    ...
    TSN Configurations:
        ...
        if self.modality in ['Flow', 'FlowNet', 'TVNet']:
    ...
    def _prepare_base_model(self, base_model):
        ...
        if self.modality == 'HP':
            ...
            self.mbpf = MotionBandPassFilter(num_segments=self.num_segments, channels=3, sigma=1.1, kernel_size=9, three_steps=True)
        ...
        if 'resnet' in base_model:
            ...
            if self.non_local:
                ...
                if base_model == 'resnet101':
                    from ops.non_local_h_101 import make_non_local
                    make_non_local(self.base_model, self.num_segments)
                elif base_model == 'resnet50':
                    from ops.rnl import make_non_local
                    make_non_local(self.base_model, self.num_segments)
                else:
                    raise Error('nonlocal only support resnet101 and 50!')
    ...
    def get_optim_policies(self):
        ...
        for name, m in self.named_modules():
            ...
            elif 'fc' in name and isinstance(m, (torch.nn.Linear, nn.BatchNorm1d, nn.SyncBatchNorm)):
                ps = list(m.parameters())
                if self.fc_lr5:
                    lr5_weight.append(ps[0])
                else:
        ...
        return [
            {'params': first_conv_weight, 'lr_mult': 5 if self.modality in ['Flow', 'FlowNet', 'TVNet'] else 1, 'decay_mult': 1,
             'name': "first_conv_weight"},
            {'params': first_conv_bias, 'lr_mult': 10 if self.modality in ['Flow', 'FlowNet', 'TVNet'] else 2, 'decay_mult': 0,
    ...
    def forward(self, input, no_reshape=False):
        if not no_reshape:
            sample_len = (3 if self.modality in ["RGB", "HP"] else 2) * self.new_length
            if self.modality == 'RGBDiff':
                sample_len = 3 * self.new_length
                input = self._get_diff(input)
            if self.modality == "HP":
                input = input.view(-1, self.num_segments, self.new_length, 3, input.size(-2), input.size(-1)) # n, t, d, c, h, w
                input = self.mbpf(input)
    ...
    def _get_diff(self, input, keep_rgb=False):
        input_c = 3 if self.modality in ["RGB", "RGBDiff", 'HP'] else 2
    ...
    def get_augmentation(self, flip=True):
        if self.modality in ['RGB', 'HP']:
            if flip:
                ...
                print('=> NO FLIP!!!')
                return torchvision.transforms.Compose([GroupMultiScaleCrop(self.input_size, [1, .875, .75, .66])])
        elif self.modality in ['Flow', 'FlowNet', 'TVNet']:
