...
for this_weights, this_test_segments, test_file in zip(weights_list, test_segments_list, test_file_list):
    if 'HP' in this_weights:
        modality = 'HP'
    elif 'RGB' in this_weights:
        modality = 'RGB'
    elif 'Flow' in this_weights:
        modality = 'Flow'
...
#     if 'Flow' not in this_weights:
#         if 'FC' in this_weights:
#             from src.model_fc import TSN
#         else:
#             from src.model_f import TSN
#         net = TSN(num_class, this_test_segments, data_length, arch, modality=modality)
#     else:
#     from ops.models import TSN
#     net = TSN(num_class, this_test_segments, modality, base_model=arch, consensus_type='avg',
#                 dropout=0.5,
#                 partial_bn=False,
#                 pretrain=None,
...
def eval_video(video_data, net, this_test_segments, modality):
    ...
    with torch.no_grad():
        ...
        num_channels = 3 if modality in ["RGB", 'HP'] else 2 
        num_channels *= data_length
        
        #n, num_crop*t, data_length*c, h, w
        data_in = data.view(batch_size, num_crop*this_test_segments, num_channels, data.size(-2), data.size(-1))
        rst = net(data_in)
