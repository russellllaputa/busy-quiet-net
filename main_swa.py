...
parser.add_argument('--modality', type=str, choices=['RGB', 'Flow', 'RGBDiff', 'HP'], default='HP')
...
parser.add_argument('--swa_freq', default=2, type=int, metavar='N',)
...
#             }, False, args)
    ...
    optimizer.bn_update(train_loader, model, device=torch.device(args.gpu))
    if rank == 0:
        save_checkpoint({
            'epoch': epoch,
            'arch': args.arch,
...
#             adjust_learning_rate(optimizer, epoch, i, args.lr_type, args.lr_steps, args, (epoch-1) + float(i) / len(train_loader))
        i += 1
        ...
        if i % args.batch_multiplier != 0:
            ...
            optimizer.step()
#             if i % (args.swa_freq * args.batch_multiplier) == 0:
#                 optimizer.update_swa()
...
def adjust_learning_rate(optimizer, epoch, step, lr_type, lr_steps, args, epoch_float=-1):
    ...
    if epoch_float < args.warmup_epochs:
        ...
        a2 = 0.00001
        c = args.swa_freq
        step = step / args.batch_multiplier
        t_i = (1 / (c-1)) * (step % c)
        lr =  (1 - t_i) * a1 + t_i * a2
        decay = args.weight_decay
    else:
