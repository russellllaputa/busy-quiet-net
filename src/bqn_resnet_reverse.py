...
class DuoPath(nn.Module):
    ...
    def __init__(self, num_segments, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, d_base_width=26, scale=4, lateral_c=False, d_stride=1, sigma=0.9, kernel_size=7, c_scale=160./256.):
        super(DuoPath, self).__init__()
        DuoPath.count += 1
        self.id = DuoPath.count
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, data_length, num_segments, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, d_base_width=26, dropout=0.5, c_scale=160./256.):
        ...
        self.down = nn.Upsample(scale_factor=c_scale, mode='bilinear', align_corners=False)
        ...
        self.layer1 = self._make_layer(DuoPath, 64, layers[0], stride=1, d_stride=1, sigma=0.9, kernel_size=7, has_lc=True, c_scale=c_scale)
        self.layer2 = self._make_layer(DuoPath, 128, layers[1], stride=2, d_stride=2, sigma=0.9, kernel_size=7, has_lc=True, c_scale=c_scale)
        self.layer3 = self._make_layer(DuoPath, 256, layers[2], stride=2, d_stride=2, sigma=0.9, kernel_size=7, has_lc=True, c_scale=c_scale)
        self.layer4 = self._make_layer(DuoPath, 512, layers[3], stride=2, d_stride=2, sigma=0.9, kernel_size=3, has_lc=True, c_scale=c_scale)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, d_stride=1, sigma=0.6, kernel_size=7, dilate=False, has_lc=False, c_scale=160./256.):
        ...
        layers.append(block(self.num_segments, self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, norm_layer,
                            lateral_c=lateral_c1, d_stride=d_stride, sigma=sigma, kernel_size=kernel_size, c_scale=c_scale))
        ...
        for _ in range(1, blocks):
            layers.append(block(self.num_segments, self.inplanes, planes, groups=self.groups, base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, lateral_c=lateral_c2, d_stride=1, sigma=sigma, kernel_size=kernel_size, c_scale=c_scale))
    ...
    def _forward_impl(self, x):
        ...
        out_c = self.avgpool(x[0])
        out_f = self.avgpool(x[1])
        
        out_c = torch.flatten(out_c, 1)
        out_f = torch.flatten(out_f, 1)

        out = torch.cat([out_c, out_f], dim=0)
