...
checkpoint_paths = {
    ...
    "x3d_m": f"pretrained/X3D_M_nofc.pth",
...
def create_x3d(
    *,
    ...
    blocks.append(head)
    return Net(blocks=nn.ModuleList(blocks), num_segments=input_clip_length, model_num_class=model_num_class)
...
class Net(nn.Module):
    ...
    def __init__(self, *, num_segments, model_num_class, blocks: nn.ModuleList) -> None:
        """
        Args:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        t = self.num_segments
        nt, dc, h, w = x.size()
        x = x.view(-1, t, 3, x.size(-2), x.size(-1)).transpose(1,2) # n, c, t, h, w
